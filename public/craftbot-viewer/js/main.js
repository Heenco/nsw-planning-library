...
function loadPicked() {
  ...
  if (!entry) return;
  // LOCAL MOD: an absolute `file` is an external model served by the app rather
  // than a file under models/, and its size is not known until it is fetched.
  const external = entry.file.startsWith("/");
  modelInfo.set(external
    ? "generated on request"
    : `<b>${entry.elements}</b> elements &middot; <b>${fmtBytes(entry.bytes)}</b>`);
  if (external) {
    document.addEventListener("craftbot:model", (ev) =>
      modelInfo.set(`<b>${ev.detail.model.count}</b> elements &middot; generated on request`),
      { once: true });
  }
  loadRunDocs(currentRun());
  loadModel(external ? entry.file : `models/${entry.file}`, { keepView: loadedExp === pick.exp });
  loadedExp = pick.exp;
}
...
async function loadRunDocs(run) {
  ...
  // LOCAL MOD: an absolute path is served by the app, not read from models/.
  const rat = run?.rationale;
  docPanel.show(rat ? (rat.startsWith("/") ? rat : `models/${rat}`) : null, data);
}
...
fetch("models/index.json", FETCH_OPTS)
  ...
  .then((idx) => {
    index = idx;
    // LOCAL MOD: a ?model= that is not in models/index.json is an external model.
    // /api/property/envelope builds one per lot on request and there are 73,595
    // Hornsby lots, so those URLs are deliberately not in the index. It is added
    // as a synthetic entry rather than loaded straight past the pickers, so the
    // MODEL dropdown names the model on screen instead of the showcase the
    // fallback would otherwise have selected.
    if (requested && requested.startsWith("/")) {
      idx.experiments.unshift({
        id: "external",
        title: bootParams.get("label") || "Generated model",
        runs: [{
          agent: "Planning controls",
          rationale: `${requested}${requested.includes("?") ? "&" : "?"}format=rationale`,
          versions: [{ v: "v01", file: requested }],
        }],
      });
    }
    ...
    for (const e of idx.experiments) {
      for (const r of e.runs) {
        for (const x of r.versions) {
          if (requested && (x.file === requested || `models/${x.file}` === requested)) found = { e, r, x };
        }
      }
    }
