...
import { resolveSiteRuleScope, fetchSiteRules } from '../utils/nsw-kg/site-rules'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // Scope and query live in site-rules.ts so /api/testing/report-inputs can
    // show exactly the rows this table is built from.
    const scope = await withNswClient(c => resolveSiteRuleScope(c, property, use))
    const proposedUse = scope.proposedUse
    const dcpLandUses = scope.landUses
    const siteRules = await withNswClient(c => fetchSiteRules(c, scope, property.lga_name))
